...
La tabla anterior revela un hallazgo clave: la media (`r round(mean(datos$gasto_total_educacion), 2)` €) es significativamente más alta que la mediana (`r round(median(datos$gasto_total_educacion), 2)` €). Esto indica una distribución asimétrica positiva (sesgada a la derecha).
...
# 1. Gráfico Lineal (como el original)
plot_gasto_linear <- ggplot(datos, aes(x = gasto_total_educacion)) +
  ...
  theme_minimal() +
  theme(legend.position = c(0.8, 0.6)) # Ajusta la posición de la leyenda si es necesario

# 2. Gráfico Logarítmico
# [cite_start]Filtramos gasto > 0 para evitar errores con log(0), ya que tu mínimo es 0 [cite: 49]
...
# 3. Combinar con patchwork (asegúrate de tener 'library(patchwork)' en el setup)
...
# 1. Gráfico de Tipo de Centro (igual que lo tenías)
...
# 2. Gráfico de Nivel Educativo (con la mejora de orden lógico)
# Definimos el orden correcto del itinerario educativo
orden_niveles <- c("Educación Infantil", 
                   "Educación Primaria", 
                   "Educación Secundaria", 
                   "Educación Superior",
                   "Otros estudios sin nivel académico") # Asegúrate que los nombres coincidan

# Filtramos solo los niveles que SÍ existen en nuestros datos
niveles_presentes <- intersect(orden_niveles, unique(datos$Nivel_estudios))

plot_nivel <- ggplot(datos, aes(x = Nivel_estudios)) + # Quitamos fct_infreq
...
# Simplemente "sumamos" los plots. Se pondrán uno al lado del otro.
...
# Gráfico de barras apiladas al 100% (o un simple pie chart)
...
analizar_gasto_complementario <- function(gasto_vector, nombre_gasto) {
  ...
  # 1. % de familias que gastan
  porc_gasta <- mean(gasto_vector > 0, na.rm = TRUE)
  
  # 2. Gasto medio de SOLO los que gastan
  media_los_que_gastan <- mean(gasto_vector[gasto_vector > 0], na.rm = TRUE)
  
  # 3. Gasto medio sobre el TOTAL (incluyendo ceros)
...
# Aplicamos la función a las variables de interés (usamos 'importe_' que limpiamos en la Sección 2)
df_matricula <- analizar_gasto_complementario(datos$importe_matricula_clases, "Matrícula/Tasas")
...
tabla_complementarios <- bind_rows(df_matricula,df_clases_part, df_comedor, df_extraescolares, df_uniformes)

knitr::kable(
  tabla_complementarios,
  caption = "Análisis de Gastos Complementarios Específicos",
  digits = 2,
  col.names = c("Tipo de Gasto", "% Estudiantes que pagan", "Gasto Medio (si paga) (€)", "Gasto Medio (Total) (€)")
) %>%
...
colores_centro <- c("Enseñanza pública" = "#0072B2", "Enseñanza concertada" = "#E69F00", "Enseñanza privada" = "#D55E00")

# Gráfico 1: Comedor
p_comedor <- ggplot(subset(datos, importe_comedor > 0), 
                    aes(x = Tipo_educacion, y = importe_comedor, fill = Tipo_educacion)) +
  geom_boxplot() +
  scale_y_continuous(labels = dollar_format(suffix = "€", prefix = "")) +
  scale_fill_manual(values = colores_centro) +
  labs(title = "Gasto en Comedor", x = NULL, y = "Gasto Anual (€)") +
  theme_minimal() +
  theme(legend.position = "none", axis.text.x = element_text(angle = 15, hjust = 1))

# Gráfico 2: Clases Particulares
p_clases_part <- ggplot(subset(datos, importe_clases_particulares > 0), 
                        aes(x = Tipo_educacion, y = importe_clases_particulares, fill = Tipo_educacion)) +
  geom_boxplot() +
  scale_y_continuous(labels = dollar_format(suffix = "€", prefix = "")) +
  scale_fill_manual(values = colores_centro) +
  labs(title = "Gasto en Clases Particulares", x = NULL, y = NULL) +
  theme_minimal() +
  theme(legend.position = "none", axis.text.x = element_text(angle = 15, hjust = 1))

# Gráfico 3: Extraescolares
p_extraescolares <- ggplot(subset(datos, importe_actividades_extraescolares > 0), 
                           aes(x = Tipo_educacion, y = importe_actividades_extraescolares, fill = Tipo_educacion)) +
  geom_boxplot() +
  scale_y_continuous(labels = dollar_format(suffix = "€", prefix = "")) +
  scale_fill_manual(values = colores_centro) +
  labs(title = "Gasto en Act. Extraescolares", x = NULL, y = "Gasto Anual (€)") +
  theme_minimal() +
  theme(legend.position = "none", axis.text.x = element_text(angle = 15, hjust = 1))

# Gráfico 4: Uniformes
p_uniformes <- ggplot(subset(datos, importe_uniformes > 0), 
                      aes(x = Tipo_educacion, y = importe_uniformes, fill = Tipo_educacion)) +
  geom_boxplot() +
  scale_y_continuous(labels = dollar_format(suffix = "€", prefix = "")) +
  scale_fill_manual(values = colores_centro) +
  labs(title = "Gasto en Uniformes", x = NULL, y = NULL) +
  theme_minimal() +
  theme(legend.position = "none", axis.text.x = element_text(angle = 15, hjust = 1))

# Combinar los 4 gráficos
(p_comedor + p_clases_part) / (p_extraescolares + p_uniformes)
```
```{r}
# Reutilizamos el orden lógico de niveles definido en la Sección 3
orden_niveles <- c("Educación Infantil", 
                   "Educación Primaria", 
                   "Educación Secundaria", 
                   "Educación Superior",
...
# 1. Gráfico de Clases Particulares por Nivel
# Calculamos la media de gasto SOLO para quienes pagan (gasto > 0)
datos_plot_clases <- datos %>%
  filter(importe_clases_particulares > 0) %>%
  group_by(Nivel_estudios) %>%
  summarise(Media_Gasto = mean(importe_clases_particulares, na.rm = TRUE))

plot_clases_nivel <- ggplot(datos_plot_clases, aes(x = Nivel_estudios, y = Media_Gasto)) +
  geom_bar(stat = "identity", fill = "#0072B2") +
  scale_x_discrete(limits = niveles_presentes) +
  scale_y_continuous(labels = dollar_format(suffix = "€", prefix = "")) +
  geom_text(aes(label = round(Media_Gasto, 0)), vjust = -0.5) +
  labs(title = "Gasto Medio en Clases Particulares (si paga)", x = NULL, y = "Gasto Anual (€)") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

# 2. Gráfico de Extraescolares por Nivel
# Calculamos la media de gasto SOLO para quienes pagan (gasto > 0)
datos_plot_extra <- datos %>%
  filter(importe_actividades_extraescolares > 0) %>%
  group_by(Nivel_estudios) %>%
  summarise(Media_Gasto = mean(importe_actividades_extraescolares, na.rm = TRUE))
  
plot_extra_nivel <- ggplot(datos_plot_extra, aes(x = Nivel_estudios, y = Media_Gasto)) +
  geom_bar(stat = "identity", fill = "#D55E00") +
  scale_x_discrete(limits = niveles_presentes) +
  scale_y_continuous(labels = dollar_format(suffix = "€", prefix = "")) +
  geom_text(aes(label = round(Media_Gasto, 0)), vjust = -0.5) +
  labs(title = "Gasto Medio en Act. Extraescolares (si paga)", x = NULL, y = NULL) +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

# Combinar
plot_clases_nivel + plot_extra_nivel
```
...
library(scales) # Para dollar_format

# 1. Crear una variable "Otros Gastos"
#    Esta es la forma correcta de hacerlo: el gasto medio total es la suma de las medias de los componentes.
datos_con_otros <- datos %>%
  mutate(
    # Calculamos 'Otros' restando los 4 complementarios al Gasto Total
    # Esto agrupa (matrícula, libros, transporte, papelería, etc.)
    importe_otros_gastos = gasto_total_educacion - 
                           (importe_clases_particulares + 
                            importe_comedor + 
                            importe_actividades_extraescolares + 
                            importe_uniformes)
  )

# 2. Calcular el GASTO MEDIO de cada componente por Tipo de Centro
#    (Se promedia sobre TODOS los estudiantes, incluidos los 0€,
#     para obtener la composición del gasto medio total).
gasto_composicion_media <- datos_con_otros %>%
  group_by(Tipo_educacion) %>%
  summarise(
    Comedor = mean(importe_comedor, na.rm = TRUE),
    Clases_Particulares = mean(importe_clases_particulares, na.rm = TRUE),
    Extraescolares = mean(importe_actividades_extraescolares, na.rm = TRUE),
    Uniformes = mean(importe_uniformes, na.rm = TRUE),
    'Otros Gastos' = mean(importe_otros_gastos, na.rm = TRUE)
  )

# 3. Convertir a formato "largo" (long format) para ggplot
gasto_composicion_long <- gasto_composicion_media %>%
  pivot_longer(
    cols = -Tipo_educacion,
...
# 4. Graficar
ggplot(gasto_composicion_long, aes(x = Tipo_educacion, y = Gasto_Medio, fill = Componente_Gasto)) +
  geom_bar(stat = "identity", position = "stack") +
  scale_y_continuous(labels = dollar_format(suffix = "€", prefix = "")) +
  labs(
    x = "Tipo de Centro",
    y = "Gasto Medio Anual por Estudiante (€)",
    fill = "Partida de Gasto"
  ) +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 10, hjust = 0.5))
```

```{r, echo=TRUE, comment="Cálculo Porcentaje Matrículas"}
# 1. Sumar el gasto total de todas las matrículas
# (El dataset 'datos' ya tiene los NA=0 gracias a la limpieza)
...
print(paste0("El gasto en matrículas (importe_matricula_clases) es un ", 
             round(porcentaje_matriculas, 2), 
             "% del gasto total."))
```

```{r, echo=TRUE, comment="Cálculo Porcentaje Alumnos que pagan Matrícula"}
# 1. Contar cuántos alumnos tienen un gasto en matrícula > 0
# (Los NAs ya fueron imputados a 0 en la limpieza)
alumnos_que_pagan <- sum(datos$importe_matricula_clases > 0, na.rm = TRUE)
...
# 4. Mostrar el resultado
print(paste0("Un ", 
             round(porcentaje_pagan, 2), 
             "% de los alumnos ('",
             alumnos_que_pagan, 
             "' de '", 
             total_alumnos,
             "') gasta en matrículas (importe_matricula_clases)."))
```
